...
| 2026-05-25 | _(ingen migration — Azure-config)_ | T5 | Backup retention 7→35 dagar + PITR-procedure | ✅ live, `backupRetentionDays=35`. Geo-redundant medvetet ej aktiverad (kostnad/nytta). PITR-drill table-top mot Azure SLA (RTO ~1-2h, RPO ~5min) — se RUNBOOK_T5.md för break-glass-procedur. |
...
| 2026-05-25 | _(ingen migration — Azure-config + .gitignore)_ | T7 | Rotera pgadmin + KV-RBAC granskning + .gitignore secret-mönster | ✅ live, pgadmin-lösenord roterat (32 URL-safe chars, ~192 bits entropi), KV-secret `database-url` uppdaterad, anslutning verifierad. KV RBAC clean (1 user-Owner + 2 ManagedIdentities-Secrets User). gitleaks i CI = follow-up. Se RUNBOOK_T7.md. |
