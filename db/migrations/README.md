...
| 2026-05-25 | _(ingen migration — Azure-config)_ | T6 | Nätverk/TLS-audit (firewall cleanup + TLS user-override) | ✅ live, 4→3 firewall-regler (duplikat borta), `require_secure_transport=on` + `ssl_min_protocol_version=TLSV1.2` båda nu user-override (var system-default). `AllowAllAzureServicesAndResourcesWithinAzureIps` behållen — verifierat load-bearing (båda App Services saknar VNet → outbound via Azure-IP-pool). Private Endpoint-migration = separat fas. Se RUNBOOK_T6.md. |
| 2026-05-25 | _(ingen migration — Azure-config)_ | T5 | Backup retention 7→35 dagar + PITR-procedure | ✅ partially met — `backupRetentionDays=35` live. Geo-redundant medvetet ej aktiverad (kostnad/nytta). **SPEC kräver "lyckat PITR-test"; vi har table-top mot Azure SLA, ej faktisk restore** (kostar $30-100/dag tills restore-instans raderas). RTO ~1-2h, RPO ~5min dokumenterade. Se RUNBOOK_T5.md för break-glass-procedur. |
| 2026-05-25 | _(ingen migration — repo-hygien)_ | T8 | Flytta extract_attachments.py + norway_saft.py ur Dropbox | ✅ live, båda filer (pre-git-föregångare till `extract.py` + `process_norway.py`) arkiverade i `C:\Users\DidWac\dev\_archive\dropbox-finance-automation-2026-05-25\`. Hash verifierad. Inga .py-filer kvar i Dropbox-roten. Se RUNBOOK_T8.md. |
| 2026-05-25 | _(ingen migration — Azure-config + .gitignore)_ | T7 | Rotera pgadmin + KV-RBAC granskning + .gitignore secret-mönster | ✅ partially met — pgadmin-lösenord roterat (32 URL-safe chars, ~192 bits entropi), KV-secret `database-url` uppdaterad, anslutning verifierad. KV RBAC clean (1 user-Owner + 2 ManagedIdentities-Secrets User). Manuell credential-scan på all git-historik: ren (alla träffar är variabel-referenser, inga klartext-värden committade). **SPEC kräver `gitleaks detect rent` som verktyg-baserat scan; manuell scan motsvarar resultat men verktyget är inte installerat. gitleaks i pre-commit + CI = follow-up.** Se RUNBOOK_T7.md. |
